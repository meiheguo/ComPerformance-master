...
from webadmin.engine.utils import get_datatables_records
from django.contrib.auth.decorators import login_required
from django.utils import simplejson
from webadmin.engine.models import LEVEL_CHOICES
from webadmin.engine.forms import BehaviorForm,DevelopmentForm,Development,DEVELOPMENT_LEVEL_CHOICES
...
def ajaxbehavior(request):
    behaviors = BehaviorForm.objects.all().reverse()
...
def deletebehavior(request):
    ...
    if request.method == "POST":
        try:
            ...
            try:
                delbehavior = BehaviorForm.objects.get(id = behavior_id)
                delbehavior.delete()
                success = True
                successinfo = "删除"
                return render_to_response('activity/behavior.html',{
                    ...
                    'success':success,
                    'username':username},context_instance = RequestContext(request))
            except BehaviorForm.DoesNotExist,e:
                traceback.print_stack()
                traceback.print_exc()
                print e
        except Exception,e:
